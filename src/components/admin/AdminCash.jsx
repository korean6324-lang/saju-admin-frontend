...
export default function AdminCash() {
    ...
    const { data: transactionsData, isLoading: isLoadingTx } = useQuery({
        ...
        queryFn: async () => {
            ...
            // 🚨 테이블명 수정 완료: coin_history (기존 cash_transactions 삭제됨)
            ...
            if (txFilter === 'point') {
                ...
                query = query.in('trade_type', ['pay', 'item_purchase', 'buy']); // P2P 구매 반영
            } else if (txFilter === 'settlement') {
                query = query.in('trade_type', ['settlement', 'withdraw', 'sell']); // P2P 판매 수익 반영
            }
